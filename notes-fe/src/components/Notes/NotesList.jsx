...
export default function NotesList({ notes, toggleChecklistItem, onDelete, onSelectNote }) {
    ...
    return (
        <div className="space-y-4 max-h-[90vh] overflow-y-auto pr-2">
            {[...notes].sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt)).map(note => (
                <NoteCard key={note._id} note={note} toggleChecklistItem={toggleChecklistItem} onDelete={onDelete} onSelectNote={onSelectNote} />
            ))}
        </div>
    );
}
